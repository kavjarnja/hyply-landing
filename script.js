...
const createOpportunityControls = () => `
  <section class="opportunity-controls" aria-labelledby="opportunity-controls-title">
    <div>
      ...
      <p data-opportunity-active-title>1 · ${(currentOutputData || createDemoOutputData()).opportunityCards[0]?.title || "Opportunity"}</p>
    </div>
...
const setupOpportunityInteractions = (root) => {
  ...
  const setActiveOpportunity = (index) => {
    ...
    if (activeTitle) {
      activeTitle.textContent = `${activeIndex + 1} · ${activeCard.title}`;
    }
  };
