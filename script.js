...
const isCustomUploadedAvatar = (avatar) => avatar?.startsWith("data:image/");
...
const syncFirebaseProfile = async (user) => {
  ...
  const avatar = isCustomUploadedAvatar(currentProfile.avatar)
    ? currentProfile.avatar
    : firebaseAvatar || currentProfile.avatar || "";
  const profile = {
    avatar,
    email: user.email || currentProfile.email,
    name: currentProfile.name === "Валентина Милашевська" && user.displayName ? user.displayName : currentProfile.name,
  };
...
const setupProfilePage = () => {
  ...
  form.addEventListener("submit", (event) => {
    ...
    const nextProfile = { ...profile, avatar, name };
