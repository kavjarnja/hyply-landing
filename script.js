...
const analyzeProjectWithApi = async (projectName, files) => {
  ...
  if (!response.ok) {
    const details =
      result?.error?.gemini?.error?.error?.message ||
      result?.error?.groq?.error?.error?.message ||
      result?.error?.groq?.error?.message ||
      result?.error?.message ||
      result?.error ||
      result?.mode ||
      `Analysis API failed with status ${response.status}`;

    throw new Error(typeof details === "string" ? details : `Analysis API failed with status ${response.status}`);
  }
