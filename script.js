...
const renderAvatarContent = (avatar, name) =>
  avatar ? `<img src="${avatar}" alt="" />` : escapeHTML(getInitials(name));
...
const setupLoggedInLandingHeader = () => {
  ...
  navActions.querySelector("[data-logout-link]")?.addEventListener("click", () => {
...
const setupProfilePage = () => {
  ...
  form.addEventListener("submit", (event) => {
    ...
    window.localStorage.setItem(profileStorageKey, JSON.stringify({ ...profile, avatar, name }));
    pendingAvatar = "";
    applyProfileState();
