...
const readableFileExtensions = ["CSV", "TXT"];
...
const createPersonaChatAnswer = (persona, answerData = null) => {
  const answer = answerData?.answer || persona.chatAnswer;
  const sources = answerData?.sources?.length
    ? answerData.sources
    : ["User Interviews", "Support Tickets", "App Store Reviews"];
  const quotes = answerData?.quotes?.length ? answerData.quotes : persona.chatQuotes;
  ...
  const tick = () => {
    ...
    window.setTimeout(async () => {
      try {
        ...
        console.warn("Analysis API is unavailable, showing demo output", error);
      }
