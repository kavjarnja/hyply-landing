      ...
      <div class="project-upload-body">
        <div class="upload-files-column">
          ...
          <div class="uploaded-files modal-uploaded-files" aria-label="Завантажені файли">
            ${createUploadedFilesMarkup(analyzedProjectFiles, false)}
          </div>
        </div>
    ...
    button.addEventListener("click", () => {
      ...
      if (action === "upload") {
        openOutputProjectModal(createUploadProjectModal(projectName), (modal) => {
          setupUploadFlow(modal, { projectNameOverride: projectName });
        });
      }
...
const setupUploadFlow = (root, { initializeFromExisting = false, projectNameOverride = "" } = {}) => {
  if (!root) {
    return;
  }
...
const createRecentProjectCard = (projectName, index) => `
  <a class="recent-project-card" href="${getProjectUrl(projectName)}">
    ...
    <small>
      <img src="https://www.figma.com/api/mcp/asset/9f0c90a9-618d-4289-acfb-ef7539eb6ed4" alt="" />
...
const renderRecentProjects = () => {
  ...
  if (projects.length <= 3) {
    if (!document.body.classList.contains("cabinet-has-projects")) {
