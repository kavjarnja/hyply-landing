...
const createLocalPersonaChatAnswerData = (persona, question) => {
  ...
  if (asksSensitiveFact) {
    return {
      answer: "Я не можу надійно відповісти на це питання. У даних немає інформації, яка дозволяє зробити навіть обережне припущення.",
  ...
  if (asksPersonalTaste) {
    return {
      answer: `Хм, мабуть, щось дуже просте. Якщо день був важкий, моя улюблена страва - це та, яку не треба довго готувати. А ще краще - яку приготував хтось інший, бо іноді хочеться не ще одного рішення, а просто видихнути.`,
  ...
  return {
    answer:
      persona.chatAnswer ||
      `${firstName} говорить насамперед про потребу швидше бачити повторювані патерни у feedback і не втрачати докази під час прийняття продуктових рішень.`,
...
const createPersonaChatPayload = (persona, question, projectName = "") => ({
  ...
  responseGuidelines: {
    ...
    instruction:
      "Answer as an evidence-backed AI persona. Use feedback as the source of truth. If the answer is inferred but not directly present, mark it as hypothetical and show a disclaimer. Never invent precise personal facts without evidence.",
    language: "uk",
  },
});
