...
const analysisFileTextLimit = 50000;
const quoteIconSvg = `
...
let unsubscribeProjectsRealtime = null;
const backfilledProjectAnalysis = new Set();
...
const inferPersonaGender = (persona = {}) => {
  if (["female", "male", "other"].includes(persona.gender)) {
    return persona.gender;
  }

  const descriptor = `${persona.name || ""} ${persona.summary || ""} ${persona.profession || ""}`.toLowerCase();

  if (/мама|жінк|користувачка|викладачка|фрілансерка|менеджерка|дослідниц|засновниц|молода|втомлена|вона|її/.test(descriptor)) {
    return "female";
  }

  if (/тато|чоловік|користувач|засновник|директор|фрилансер|викладач|він|його/.test(descriptor)) {
    return "male";
  }
...
const createPersonaChatUnavailableAnswerData = () => ({
  answer:
    "Не вдалося отримати відповідь від AI-моделі. Перевір, будь ласка, чи оновлений Cloudflare Worker і чи доступний API, бо я більше не підставляю локальні шаблони замість справжньої розмови.",
  answerType: "insufficient_data",
  confidence: "low",
  disclaimer: "Відповідь не була згенерована моделлю.",
  relatedSignals: [{ signal: "Persona Chat має відповідати через AI Worker, без локальних шаблонів.", source: "System status" }],
  supportingQuotes: [],
});
...
const createPersonaChatPayload = (persona, question, projectName = "", conversationHistory = []) => ({
  ...
  persona: {
    ...
    field: persona.field,
    goals: persona.goals || [],
    gender: inferPersonaGender(persona),
...
const askPersonaWithApi = async (persona, question, projectName = "", conversationHistory = []) => {
  if (!hyplyApiEndpoint) {
    throw new Error("Persona Chat API endpoint is not configured");
  }
  ...
  if (!String(result.answer || "").trim()) {
    throw new Error("Persona Chat API returned an empty answer");
  }
    ...
    answerTimer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (looksLikePersonaSummaryAnswer(persona, answerData, message)) {
          throw new Error("Persona Chat returned a static persona summary instead of a model conversation");
        }
      } catch (error) {
        console.warn("Persona Chat API is unavailable or returned a static summary", error);
        answerData = createPersonaChatUnavailableAnswerData();
      }
  ...
  return `
    <section class="opportunity-chart opportunity-rice" aria-labelledby="rice-title">
      ...
      <div class="rice-legend" aria-hidden="true">
        ...
        <span><i class="rice-effort"></i>Effort</span>
      </div>
    </section>
  `;
};
...
const createOpportunityCard = (card, index) => `
  <article class="opportunity-card ${index === 0 ? "is-active" : ""}" data-opportunity-card="${index}" role="button" tabindex="0" aria-label="Обрати можливість ${index + 1}: ${card.title}">
    <div class="opportunity-card-head">
      ...
      <h3 class="opportunity-card-title">${card.opportunity}</h3>
    </div>
    <div class="opportunity-problem-row">
      <span class="opportunity-problem-icon" aria-hidden="true">
        <svg viewBox="0 0 20 20" fill="none">
          <circle cx="10" cy="10" r="8" stroke="#FF4D45" stroke-width="1.6"/>
          <path d="M10 5.5v5.7" stroke="#FF4D45" stroke-width="1.6" stroke-linecap="round"/>
          <path d="M10 14.45h.01" stroke="#FF4D45" stroke-width="2.1" stroke-linecap="round"/>
        </svg>
      </span>
      <p>${card.problem}</p>
    </div>
    <div class="opportunity-card-divider" aria-hidden="true"></div>
...
const createOpportunityControls = () => `
  <section class="opportunity-controls" aria-labelledby="opportunity-controls-title">
    <div>
      ...
      <p class="opportunity-active-meta">
        <b data-opportunity-active-number>1</b>
        <span data-opportunity-active-title>${(currentOutputData || createDemoOutputData()).opportunityCards[0]?.title || "Opportunity"}</span>
      </p>
    </div>
...
const setupOpportunityInteractions = (root) => {
  ...
  const activeNumber = opportunityRoot.querySelector("[data-opportunity-active-number]");
  let activeIndex = 0;
  ...
  const setActiveOpportunity = (index) => {
    ...
    if (activeNumber) {
      activeNumber.textContent = activeIndex + 1;
    }
  };
    ...
    button.addEventListener("click", () => {
      ...
      if (action === "edit") {
        openOutputProjectModal(createEditProjectModal(projectName), (modal) => {
          ...
          form.addEventListener("submit", async (event) => {
            ...
            renameProjectLocally(projectName, nextProjectName);
            await renameProjectInFirestore(projectName, nextProjectName);
      ...
      if (action === "delete") {
        openOutputProjectModal(createDeleteProjectModal(), (modal) => {
          modal.querySelector("[data-confirm-delete-project]").addEventListener("click", async () => {
            ...
            await deleteProjectFromFirestore(projectName);
...
const showOutputView = (projectName) => {
  ...
  if (getStoredProjectAnalysis(projectName) && !backfilledProjectAnalysis.has(projectName)) {
    backfilledProjectAnalysis.add(projectName);
...
const renameProjectAnalysis = (projectName, nextProjectName) => {
  if (!projectName || !nextProjectName || projectName === nextProjectName) {
    return;
  }
...
const renameProjectLocally = (projectName, nextProjectName) => {
  if (!projectName || !nextProjectName || projectName === nextProjectName) {
    return;
  }

  renameProjectAnalysis(projectName, nextProjectName);
...
const createOutputDataFromAnalysis = (projectName) => {
  ...
  const normalizedPersonas = (analysis.personas || []).map((persona, index) => {
    ...
    const normalizedPersona = {
      ...
      gender: ["female", "male", "other"].includes(persona.gender) ? persona.gender : inferPersonaGender(persona),
      goals,
...
const createAnalysisPayload = (projectName, files) => ({
  files: files.map((file) => ({
    ...
    name: file.name,
    size: file.size || 0,
    text: String(file.text || "").slice(0, analysisFileTextLimit),
    textLength: file.textLength || file.text?.length || 0,
    textTruncated: Boolean(file.textTruncated) || String(file.text || "").length > analysisFileTextLimit,
    type: file.type || getFileExtension(file.name),
  })),
  projectName,
...
const analyzeProjectWithApi = async (projectName, files) => {
  ...
  try {
    response = await fetch(hyplyApiEndpoint, {
    ...
    throw new Error(
      "Не вдалося з'єднатися з AI API. Перезавантаж сторінку, перевір інтернет і спробуй ще раз. Якщо сайт відкритий локально, переконайся, що Cloudflare Worker задеплоєний з актуальним CORS-фіксом.",
    );
  }
  ...
  if (!response.ok) {
    const details = result?.error?.error?.message || result?.error?.message || result?.error || result?.mode || "";
    const errorText = typeof details === "string" ? details : "";
    const isQuotaError =
      response.status === 429 ||
      /quota|rate.?limit|resource_exhausted|too many requests|вичерпан/i.test(errorText) ||
      result?.error?.error?.status === "RESOURCE_EXHAUSTED";

    throw new Error(
      isQuotaError
        ? "Квота AI-аналізу тимчасово вичерпана. Будь ласка, спробуйте ще раз пізніше або після оновлення ліміту Gemini."
        : errorText || `Analysis API failed with status ${response.status}`,
    );
  }
  ...
  const tick = () => {
    ...
    window.setTimeout(async () => {
      try {
        ...
        document.body.classList.remove("is-analysis-running");
        const errorMessage = String(error?.message || "").slice(0, 220);
        window.sessionStorage.setItem(
          projectToastStorageKey,
          errorMessage
            ? `Не вдалося проаналізувати файли: ${errorMessage}`
...
const renameProjectInFirestore = async (oldProjectName, nextProjectName) => {
  renameProjectLocally(oldProjectName, nextProjectName);
...
const syncProjectsFromSnapshot = (snapshot) => {
  const previousProjects = readCreatedProjects();
  const analysisMap = readProjectAnalysisMap();
  const filesMap = readProjectFilesMap();
  const remoteProjects = [];
  const remoteProjectSet = new Set();
  ...
  snapshot.docs.forEach((doc) => {
    ...
    remoteProjects.push(projectName);
    remoteProjectSet.add(projectName);
  ...
  previousProjects.forEach((projectName) => {
    if (!remoteProjectSet.has(projectName)) {
      delete analysisMap[projectName];
      delete filesMap[projectName];
    }
  });
  ...
  window.localStorage.setItem(projectAnalysisStorageKey, JSON.stringify(analysisMap));
  window.localStorage.setItem(projectFilesStorageKey, JSON.stringify(filesMap));
  writeCreatedProjects([...new Set(remoteProjects)].slice(0, 12));
  renderSidebarProjects();
  ...
  if (!remoteProjectSet.has(requestedProjectName)) {
    window.sessionStorage.setItem(projectToastStorageKey, `Проєкт "${requestedProjectName}" видалено`);
    window.location.href = "./cabinet-no-projects.html";
    return;
  }
...
const startProjectsRealtimeSync = async () => {
  ...
  if (unsubscribeProjectsRealtime) {
    unsubscribeProjectsRealtime();
    unsubscribeProjectsRealtime = null;
  }

  try {
    unsubscribeProjectsRealtime = projectsCollection.orderBy("updatedAt", "desc").onSnapshot(
      syncProjectsFromSnapshot,
      (error) => {
        console.warn("Projects realtime sync failed", error);
        loadProjectsFromFirestore();
      },
    );
  } catch (error) {
    console.warn("Projects realtime sync was not started", error);
...
const setupFirebaseAuthState = () => {
  ...
  auth.onAuthStateChanged(async (user) => {
    ...
    await startProjectsRealtimeSync();
    const requestedProjectName = getProjectNameFromUrl();
