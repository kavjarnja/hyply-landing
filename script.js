...
const figmaAssets = {
  ...
  hypothesisQuoteIcon: "https://www.figma.com/api/mcp/asset/d2ea33ff-213b-4ddc-8d54-2bffc3d2ce88",
  evidenceQuoteIcon: "https://www.figma.com/api/mcp/asset/e839a1a1-80b9-4524-a84a-611220d315d9",
  deleteProjectIllustration: "https://www.figma.com/api/mcp/asset/5499dae0-9d7b-4c02-bbfa-6032faf75223",
};
...
let analyzedProjectFiles = [
  {
    displaySize: "22 KB",
    name: "IT Ukraine Association.txt",
    size: 0,
  },
  {
    displaySize: "22 KB",
    name: "IT Ukraine Association.docx",
    size: 0,
  },
];
...
const createUploadedFilesMarkup = (files) =>
  files
    .map(
      (file, index) => `
    ...
    <section class="project-upload-modal" role="dialog" aria-modal="true" aria-labelledby="output-upload-title">
      ...
      <div class="project-upload-body">
        <div class="upload-files-column">
          ...
          <div class="uploaded-files modal-uploaded-files" aria-label="Завантажені файли">
            ${createUploadedFilesMarkup(analyzedProjectFiles)}
          </div>
        </div>
        ...
        <div class="project-form modal-project-form">
          <label for="output-upload-project-name">Назва проєкту</label>
          <div class="modal-project-actions">
            <input id="output-upload-project-name" type="text" value="${escapeHTML(projectName)}" />
            <button type="button" ${analyzedProjectFiles.length ? "" : "disabled"}>
              ${createAnalyzeIcon()}
              Проаналізувати
            </button>
          </div>
          ...
          ? `
            <div class="voc-details">
              ...
              <section class="voc-quotes">
                <h3>ЦИТАТИ</h3>
                ${problem.quotes.map((quote) => `<blockquote><span aria-hidden="true">“</span>${quote}</blockquote>`).join("")}
              </section>
            </div>
          `
          : ""
              ...
              <section class="hypothesis-evidence">
                ...
                ${hypothesis.quotes
                  .map(
                    (quote) => `
                      <blockquote>
                        <img src="${figmaAssets.hypothesisQuoteIcon}" alt="" />
                        ${quote}
                      </blockquote>
                    `,
                  )
  ...
  <article class="evidence-card">
    <div class="evidence-quote">
      <span class="evidence-quote-icon" aria-hidden="true">
        <img src="${figmaAssets.evidenceQuoteIcon}" alt="" />
      </span>
      <p>${item.quote}</p>
    </div>
...
const renderOutputView = (
  ...
  cabinetMain.querySelectorAll("[data-output-tab]").forEach((button) => {
    button.addEventListener("click", () => {
      renderOutputView(cabinetMain, projectName, button.dataset.outputTab, null, null, 0);
    });
  });
    ...
    button.addEventListener("click", () => {
      ...
      if (action === "upload") {
        openOutputProjectModal(createUploadProjectModal(projectName), (modal) => {
          setupUploadFlow(modal, { initializeFromExisting: true });
          const input = modal.querySelector("#output-upload-project-name");
...
const setupUploadFlow = (root, { initializeFromExisting = false } = {}) => {
  ...
  analyzeButton.addEventListener("click", () => {
    if (!analyzeButton.disabled) {
      ...
      analyzedProjectFiles = uploadedFilesState.map((file) => ({ ...file }));
...
const getSeedSidebarProjects = (projectSection) => {
  const existingList = projectSection.querySelector(".cabinet-project-list");

  if (!existingList) {
    return [];
  }

  if (projectSection.dataset.seedProjects) {
    try {
      return JSON.parse(projectSection.dataset.seedProjects);
    } catch {
      return [];
    }
  }

  const projects = Array.from(existingList.querySelectorAll(".cabinet-project-link"))
    .map((link) => link.textContent.trim())
    .filter(Boolean);

  projectSection.dataset.seedProjects = JSON.stringify(projects);

  return projects;
};

const createSidebarProjectLink = (projectName) => `
  <a href="./cabinet-have-projects.html" class="cabinet-project-link">
    <span aria-hidden="true">
...
const renderSidebarProjects = () => {
  ...
  const seedProjects = document.body.classList.contains("cabinet-has-projects") ? getSeedSidebarProjects(projectSection) : [];
  const projects = [...new Set([...createdProjects, ...seedProjects])];
